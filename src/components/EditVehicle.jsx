...
export default function EditVehicle({ vehicle, onClose, onUpdateVehicle, branches }) {
  ...
  const [formData, setFormData] = useState({
    ...
    licenseExpiry: formatDate(vehicle?.licenseExpiry),
    insuranceExpiry: formatDate(vehicle?.insuranceExpiry),
    branchId: vehicle?.branchId || '',
    originalBranchId: vehicle?.branchId || '',
    make: vehicle?.make || '',
    model: vehicle?.model || '',
  ...
  const [uploadedDocuments, setUploadedDocuments] = useState({
    licenseDocument: vehicle?.licenseDocument || null,
    insuranceDocument: vehicle?.insuranceDocument || null
  });
  ...
  const handleUploadClick = async (documentType) => {
    ...
    formDataUpload.append('vehicleId', 'temp-upload');
  ...
  const validateForm = () => {
    const newErrors = {};
    // Only license number is absolutely required
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      console.log('Edit - Uploaded documents state:', uploadedDocuments);
      console.log('Edit - Original vehicle documents:', {
      ...
      const updatedVehicle = {
        ...formData,
        branch: formData.branchId, // Map branchId to branch for the API
        licenseDocument: uploadedDocuments.licenseDocument,
        insuranceDocument: uploadedDocuments.insuranceDocument
      };
      
      console.log('Edit - Vehicle data being sent:', updatedVehicle);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              className={`mt-1 block w-full border ${errors.licenseExpiry ? 'border-red-300' : 'border-gray-300'} rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm`}
            />
          ...
          <div>
            ...
            {uploadedDocuments.licenseDocument && (
              <div className="mt-1 text-xs text-green-600">
                ✓ Current: {uploadedDocuments.licenseDocument.fileName}
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              className={`mt-1 block w-full border ${errors.insuranceExpiry ? 'border-red-300' : 'border-gray-300'} rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm`}
            />
            <p className="mt-1 text-xs text-gray-500">dd/mm/yyyy</p>
          ...
          <div>
            ...
            {uploadedDocuments.insuranceDocument && (
              <div className="mt-1 text-xs text-green-600">
                ✓ Current: {uploadedDocuments.insuranceDocument.fileName}
              </div>
            )}
          </div>
